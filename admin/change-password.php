...
if($_POST){
  ...
  $npass = $_POST['npass'];
  $cpass = $_POST['cpass'];
  $uid = $_SESSION['s_admin_id'];
  //fetch old password

  $oq = mysqli_query($connection, "SELECT * FROM tbl_admin WHERE admin_id = '{$uid}'");
  ...
  if( $odata['admin_password'] == $opass)
  {
    if($npass == $cpass)
    {
      if($opass == $npass)
      {
        ...
        //update query
        $uq = mysqli_query($connection,"update tbl_admin set admin_password = '{$npass}' where admin_id = '{$uid}'");

        echo "<script>alert(' password Updated');window.location='home.php';</script>"; 
      }
    }
    else
    {
