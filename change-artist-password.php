...
if($_POST){
  ...
  $oq = mysqli_query($connection, "SELECT * FROM tbl_artist WHERE artist_id = '{$aid}'");

  $odata = mysqli_fetch_array($oq);

  if( $odata['artist_password']==$opass)
  {
    if($npass == $cpass)
    {
      if($opass == $npass)
      {
        ...
        //update query
        $uq = mysqli_query($connection,"update tbl_artist set artist_password = '{$npass}' where artist_id = '{$aid}'");
