...
<div id="aon-provider-review" class="sf-provi-articles-box margin-b-50 sf-provi-fullBox  " >
                    <h3 class="sf-provi-title "> Customer Feedback</h3>
                    ...
                    <?php
                     if(isset($_GET['did']))
                     {
                         $q = mysqli_query($connection,"delete from tbl_feedback where feedback_id ='{$_GET['did']}'");
                     }
$fq = mysqli_query($connection, "SELECT * FROM tbl_feedback where user_id ='{$_SESSION['uid']}' ");
...
while ($fdata = mysqli_fetch_array($fq)){
    ...
    echo "<tr>";
    echo "<td>{$fdata['feedback_id']}</td>";
    echo "<td>{$udata['artist_name']}</td>";
    echo "<td>{$fdata['feedback_date']}</td>";
    echo "<td>{$fdata['feedback_details']}</td>";
 echo "<td><a href='?did={$fdata['feedback_id']}'>Delete</a></td>";
